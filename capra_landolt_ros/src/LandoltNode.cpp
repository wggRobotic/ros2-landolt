...
{
public:
    explicit LandoltNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
    : Node("landolt_detector", options)
    {
        this->declare_parameter<int>("queue_size", 5);
        this->declare_parameter<std::string>("camera_reading", "/camera_3d/rgb/image_raw");

        queue_size_ = this->get_parameter("queue_size").as_int();
        camera_reading_ = this->get_parameter("camera_reading").as_string();

        RCLCPP_INFO(this->get_logger(), "Subscribing to: %s", camera_reading_.c_str());

        landolt_pub_ = this->create_publisher<capra_landolt_msgs::msg::Landolts>("landolts", rclcpp::QoS(10));
        bounding_pub_ = this->create_publisher<capra_landolt_msgs::msg::BoundingCircles>("boundings", rclcpp::QoS(10));
        image_pub_ = this->create_publisher<sensor_msgs::msg::Image>("image", rclcpp::QoS(1));
...
private:
    ...
    void imageCb(const sensor_msgs::msg::Image::SharedPtr image_msg)
    {
        ...
        if (image_pub_->get_subscription_count() > 0)
        {
            ...
            hdr.stamp = now;
            cv_bridge::CvImage img_msg(hdr, sensor_msgs::image_encodings::BGR8, img_ptr->image);
